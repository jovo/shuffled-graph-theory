...
Let $\GG'=\QQ(\GG): \Omega \to \mc{G}_n'$ be a \emph{shuffled} graph-valued random variable, that is, a labeled graph valued random variable that has been passed through a shuffle channel $\QQ$. Note that $|\mc{G}_n'|=|\mc{G}_n|$. % taking values $G\in \mc{G}_n$, where $\mc{G}_n$ is the set of labeled graphs on $n$ vertices.	
\end{defi}
