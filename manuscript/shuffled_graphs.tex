...
% $\bth_y=[\theta_{1|y},\ldots \theta_{d|y}]\T \in \triangle_d$, where  $d=d_n=2^{\binom{n}{2}}$ and $\triangle_d$ is the $d$-dimensional simplex (that is, $\theta_{i|y}\geq 0$ for all $i \in [d]$ and $\sum_{i \in [d]} \theta_{i|y}=1$). 
...
%, $\mt{\bth}=\{\mt{\bth}_y\}_{y\in \mc{Y}}$ and $\mb{\pi}=\{\pi_y\}_{y \in \mc{Y}}$. 
...
% one could simply take the shuffled graph data $\mc{T}_s'$, without using $\psi$ (as defined above), and estimate $\bth_y'$.  While silly (as we know that ), this procedure is still asymptotically optimal.  Let $\mh{L}_s'$ be the risk of the induced \emph{shuffled} graph Bayes plugin classifier obtaining maximum likelihood estimates from $\mc{T}_s'$ to obtain the plugin estimate for Eq. \eqref{eq:Sh_Plug}. As above,  it follows that
...
\begin{thm} \label{thm:inadmiss}
	$\mh{h}_s'$ is inadmissible under $0-1$ loss. 
\end{thm}
\begin{proof}
	Assume that there exists at least one unlabeled set $\mt{G}$ with two elements, $G_1, G_2$, such that $\mt{\theta}_{\mt{G}|y_i}\neq \mt{\theta}_{\mt{G}|y_j}$ for  $y_i \neq y_j$.  $\mh{h}'_s$ depends on estimating $\theta'_{G_1|y}$ and $\theta'_{G_2|y}$ for $y\in\mc{Y}$, whereas $\mh{\mt{h}}_s$ depends on only estimating $\mh{\mt{\theta}}_{\mt{G}|y}$ for $y \in \mc{Y}$. Now consider two scenarios.  In the first scenario, assume that all training data are $G_1$.  If the test shuffled graph is also $G_1$, the estimates, $\mh{\theta}'_{G_1|y}$ and $\mh{\mt{\theta}}_{\mt{G}|y}$ for $y \in \mc{Y}$, yield the same answer.  However, if the test shuffled graph is $G_2$, the shuffled plugin classifier $\mh{h}'_s$ will randomly assign a class, whereas the unshuffled plugin classifier $\mh{\mt{h}}_s$ will classify according to the maximum a posteriori class appropriately (and will have lower variance).  In the second scenario, training data include both $G_1$ and $G_2$.  
