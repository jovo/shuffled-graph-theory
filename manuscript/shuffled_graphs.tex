...
For \emph{labeled} graph classification, $\PP_{\QQ}$ is assumed to be the identity function, therefore, $\mc{T}_s=\{(\GG_i,Y_i)\}_{i \in [s]}$, because $\QQ_i(\GG_i)=\GG_i$ when graphs are labeled.
...
$\mh{\mt{h}}_s: \mt{\mc{G}}_n \times \mc{T}_s' \to \mc{Y}$,
