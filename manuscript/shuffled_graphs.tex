...
\begin{proof}
	...
	Both $\mh{h}_s^{\delta}$ and $\mh{h}_s^{'\delta'}$ operate on $\mc{T}_s'$, that is, shuffled graph data.  Let $knn_{\delta}(G')$ be the collection of $k_s$ nearest neighbors of $G'$ using $\delta$ as the distance metric.  Let $knn_{\delta'}(G')$ be the same except using $\delta'$ as the distance metric.  Let $\{G_i\}_{\forall y_i=y_{G'}}$ indicate the set of training graphs with the same class label as the test graph, $G'$.  
